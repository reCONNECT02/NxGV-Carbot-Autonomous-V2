...
| 05 local pose: ICP + IMU lead | `run_icp_eval.py` | four scenarios (nominal, `imu_drift`, `map_offset`, `noisy`) x four modes (odom / map / icp / both): position, sideways and along-track error, heading error, largest single step, ICP accepted/attempted and CPU per update. `--set icp.gain=1.0 heading_lead_s=0.35` overrides any `local_pose` key. Results: `docs/LOCALIZATION_MEMORY.md`. |
