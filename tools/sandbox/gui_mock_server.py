...
"""GUI mock server: the real carbot_gui web/ files with synthetic data, no ROS.
    ...
    python3 tools/sandbox/gui_mock_server.py [--mode race|calibrate] [--scenario drive|stop] [--port 8081]
                                             [--sensors ok|bad] [--data-root DIR] [--unlock-to N]
...
rotated by MOCK_LIDAR_DELTA_DEG; --unlock-to 5 marks steps 1-4 passed (in memory
only) so its Run can be tried.
...
class MockWizard:
    ...
    def __init__(self, sensors, root, unlock_to=0):
        import yaml
        ...
        for sl in self.wiz.slots:                  # mock only: pretend the earlier steps passed
            if sl.index < unlock_to and sl.required:
                sl.status = sl.saved_status = 'PASS'
...
def main():
    ...
    ap.add_argument('--unlock-to', type=int, default=0, help='calibrate: mark steps below N passed (mock only)')
    a = ap.parse_args()
    mock = Mock(a.mode, a.scenario)
    if a.mode == 'calibrate':
        import tempfile
        root = a.data_root or tempfile.mkdtemp(prefix='carbot_mock_data_')
        mock.wiz = MockWizard(a.sensors, root, a.unlock_to)
