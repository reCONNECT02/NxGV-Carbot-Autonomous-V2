"""UWB range parser (feeds BLOCK 06 only; UWB never reaches the servo).
...
Out: /carbot/uwb/ranges   UwbRanges, one per tag report (empty = no measurement)
     ...
     /carbot/uwb/raw_fix  PointStamped (venue) pairwise trilateration, GUI/calibration
                          only; block 06 uses the per-anchor ranges, not this.
...
from .uwb_core import AnchorSet, RangeProcessor, parse_report, trilaterate

REQUIRED = ['link_timeout_s', 'anchor_timeout_s', 'expected_rate_hz', 'max_range_age_ms',
            'min_range_m', 'max_range_m', 'publish_raw_fix', 'status_publish_hz',
...
class UwbRangesNode(CarbotNode):
    ...
    def _on_json(self, msg: String) -> None:
        ...
        if self.publish_fix:
            ...
            if p is not None:
                f = PointStamped()
                f.header.stamp = out.header.stamp
                f.header.frame_id = self.venue
                f.point.x, f.point.y = float(p[0]), float(p[1])
    ...
    def _status(self) -> None:
        ...
        if not link_ok:
            ...
            self.set_status(lvl, 'RUNNING', f'{rate:.1f} Hz, {len(self.anchors.ids)} anchors, '
                            f'latency {self.latency_ms:.0f} ms, bad json {self.bad_json}')
