...
| 21 | KEEP_PREVIOUS for data-writing steps must copy their data files | 8 | DOING | Hook `StepImpl.keep_data` exists; step 2 implements it (refuses when the old confirmation misses a camera enabled now); step 5 too (cameras.yaml lidar_to_camera + both overlay keys). Each later data-writing page must too |
